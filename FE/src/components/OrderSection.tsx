...
export default function OrderSection({
  ...
  useEffect(() => {
    reloadData().catch((err: any) =>
      setCreationError(err.message || "Unable to load order data"),
    );
  }, [refreshSignal]);
  ...
  const removeOrderRow = (index: number) => {
    if (orderRows.length === 1) return; // Keep at least one row
  ...
  const handleCreateOrderSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedCustomerId) {
      return setCreationError("Please select a customer for this order");
    }

    // Filter empty rows
    const validRows = orderRows.filter((row) => row.productId !== "");
    if (validRows.length === 0) {
      return setCreationError("Please select at least one valid product");
    }

    // Check duplicate products in rows
    const selectedIds = validRows.map((r) => r.productId);
    const hasDuplicates = selectedIds.some(
      (val, i) => selectedIds.indexOf(val) !== i,
    );
    if (hasDuplicates) {
      return setCreationError(
        "Duplicate products found in row items. Please consolidate lines.",
      );
    }
    ...
    if (hasInvalidQty) {
      return setCreationError(
        "All product lines must have an ordered quantity greater than zero",
      );
    }
    ...
    try {
      ...
      setCreationSuccess(
        `Order filed with invoice ID "${submittedOrder.id}" successfully.`,
      );
      ...
      await reloadData();
      onRefresh();
      setTimeout(() => setCreationSuccess(""), 5000);
    } catch (err: any) {
      setCreationError(
        err.message || "Verification exception occurred placing order",
      );
    }
  };
  ...
  const handleCancelOrder = async (id: string) => {
    if (
      window.confirm(
        "Cancel order? Re-allocated stocks will automatically cycle back into product totals.",
      )
    ) {
      try {
        ...
        setCreationError(err?.message || "Error cancelling");
      }
    }
  };
  ...
  const handleDeleteOrder = async (id: string, e: React.MouseEvent) => {
    ...
    if (
      window.confirm(
        "Delete order record entirely? Stocks will automatically cycle back into product inventory. Continue?",
      )
    ) {
      try {
        await OrderService.delete(id);
        ...
        setCreationError(err?.message || "Error deleting");
      }
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in" id="order-management-section">
      {/* Tab Selectors Row */}
      ...
      {creationSuccess && (
        <div className="flex items-center gap-2.5 p-3.5 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-lg text-xs shadow-xs animate-slide-up">
      ...
      {activeTab === "list" && (
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search invoices by invoice reference UUID, customer name, email address..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-9 pr-4 py-2 text-sm bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 shadow-2xs"
            />
          </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-4 text-center">
                            <span
                              className={`inline-flex items-center gap-1.5 px-2.1 py-0.5 text-[9px] font-bold rounded-full border ${
                                isCompleted
                            ...
                            >
                              <span
                                className={`w-1 h-1 rounded-full ${isCompleted ? "bg-emerald-500" : "bg-rose-500"}`}
                              />
      ...
      {activeTab === "create" && (
        <form
          ...
          className="bg-white border border-slate-200 rounded-xl shadow-3xs p-6 space-y-6 animate-slide-up text-xs"
        >
          ...
          {creationError && (
            <div className="p-3 bg-rose-50 border border-rose-100 text-rose-800 rounded-lg font-medium flex items-center gap-1.5 animate-pulse">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              required
              className="w-full text-sm py-2 px-3 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 font-medium text-slate-800"
            >
          ...
          <div className="space-y-3.5">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={addOrderRow}
                className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-bold text-blue-600 border border-blue-200 hover:bg-blue-50 rounded-md transition-colors"
              >
                <Plus className="w-3.5 h-3.5" />
                Add Item Row
              </button>
            ...
            <div className="space-y-3">
              {orderRows.map((row, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex flex-col sm:flex-row gap-3 items-end p-3 bg-slate-50 border border-slate-200 rounded-lg relative group"
                  >
                    ...
                    <div className="flex-1 space-y-1 w-full min-w-0">
                      ...
                      <select
                        ...
                        required
                        className="w-full text-sm bg-white border border-slate-200 p-2 rounded-md focus:outline-none focus:border-blue-500 font-medium text-slate-700 truncate"
                      >
                    ...
                    {currentSelectedProduct && (
                      <div className="text-[10px] font-medium min-w-[70px] self-center pt-3 text-center">
                        <span className="text-slate-400">Available Stock:</span>
                        <p
                          className={`font-mono font-bold ${currentSelectedProduct.quantity === 0 ? "text-rose-600" : "text-slate-700"}`}
                        >
                    ...
                    <div className="space-y-1 w-full sm:w-[100px] shrink-0">
                      ...
                      <input
                        ...
                        className={`w-full p-2 bg-white border rounded-md font-mono text-center text-sm focus:outline-none ${
                          isOverStock
                    ...
                    <button
                      ...
                      className="p-2 self-center rounded text-slate-400 hover:text-rose-500 hover:bg-slate-200 transition-colors disabled:opacity-40"
                      title="De-select line item"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>

                    {/* Instant live exceed warning */}
                    {isOverStock && currentSelectedProduct && (
                      <span className="absolute bottom-1 right-2 bg-rose-500 text-white font-bold text-[9px] px-1.5 py-0.5 rounded-sm animate-pulse">
          ...
          <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <button
                ...
                className="flex-grow sm:flex-initial inline-flex items-center gap-1.5 justify-center py-2.5 px-6 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-sm disabled:opacity-50 transition-colors"
                id="btn-submit-order"
              >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 pb-4 border-b border-slate-100 text-[11px]">
                ...
                <div className="space-y-1.5 text-right">
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 px-2 py-0.5 mt-0.5 text-[10px] font-bold rounded-full ${
              ...
              <div className="flex items-center gap-2 pt-2 text-[11px]">
                <button
                  type="button"
                  onClick={() => setViewingOrder(null)}
                  className="flex-1 py-2 text-slate-600 font-semibold border border-slate-200 rounded-lg bg-slate-50 hover:bg-slate-150"
                >
                  Close Receipt
                </button>
                {viewingOrder.status === "Completed" && (
                  <button
                    ...
                    className="flex-1 py-2 bg-rose-50 text-rose-700 font-bold border border-rose-200 rounded-lg hover:bg-rose-100 flex items-center justify-center gap-1 transition-colors"
                  >
