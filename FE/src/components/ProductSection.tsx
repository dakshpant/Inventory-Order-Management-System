...
export default function ProductSection({
  ...
  const [filter, setFilter] = useState<"all" | "low" | "out" | "safe">("all");
  ...
  const handleDelete = async (p: Product) => {
    ...
    const isInOrders = orders.some((o) =>
      o.items.some((it) => it.productId === p.id),
    );
  ...
  const filteredProducts = products.filter((p) => {
    ...
    if (filter === "safe") return p.quantity >= 10;
    return true; // 'all'
  });

  return (
    <div className="space-y-6 animate-fade-in" id="product-management-section">
      {/* Tab Header Row */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 justify-center px-4 py-2 text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-xs transition-colors"
          id="btn-add-product"
        >
      ...
      {/* Success Banner */}
      {success && (
        <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-lg text-xs shadow-xs animate-slide-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-2xs">
          ...
          <select
            ...
            className="w-full text-xs bg-transparent border-none focus:outline-none text-slate-600 font-medium"
          >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-center">
                        {isOut ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-full bg-rose-50 text-rose-700 border border-rose-100">
                            Depleted Stock
                          </span>
                        ) : isLow ? (
          ...
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  placeholder="e.g. Premium Titanium Gear Rods"
                  required
                  value={name}
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  placeholder="e.g. TIT-GEAR-101"
                  required
                  value={sku}
                  onChange={(e) => setSku(e.target.value)}
                  disabled={!!editingId} // Usually best not to edit SKU directly once established
