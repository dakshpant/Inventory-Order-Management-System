...
export default function OrderSection({ onRefresh, refreshSignal, }) {
    ...
    useEffect(() => {
        reloadData().catch((err) => setCreationError(err.message || "Unable to load order data"));
    }, [refreshSignal]);
    ...
    const handleCreateOrderSubmit = async (e) => {
        ...
        if (hasDuplicates) {
            return setCreationError("Duplicate products found in row items. Please consolidate lines.");
        }
        // Validate quantities positive
        const hasInvalidQty = validRows.some((row) => row.quantity <= 0);
        if (hasInvalidQty) {
            return setCreationError("All product lines must have an ordered quantity greater than zero");
        }
        try {
            ...
            setCreationSuccess(`Order filed with invoice ID "${submittedOrder.id}" successfully.`);
            ...
            setCreationError(err.message || "Verification exception occurred placing order");
        }
    };
    ...
    return (<div className="space-y-6 animate-fade-in" id="order-management-section">
      ...
      {creationSuccess && (<div className="flex items-center gap-2.5 p-3.5 bg-emerald-50 border border-emerald-100 text-emerald-800 rounded-lg text-xs shadow-xs animate-slide-up">
                ...
                <div className="border border-slate-150 rounded-lg overflow-hidden bg-slate-50/50">
                  <table className="w-full text-left font-medium">
                    ...
                    <tbody className="divide-y divide-slate-150 text-[11px] text-slate-700 font-medium">
                      {viewingOrder.items.map((it, i) => (<tr key={i}>
